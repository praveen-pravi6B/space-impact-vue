import { Player, Explosion, Particle, UI, Projectile, Missile, Laser, Wall } from './entities.js';
import {
...
class Level {
  ...
  enemyDead(enemy) { this.explosions.push(new Explosion(this.game, enemy.x, enemy.y)); this.game.playSound('explosion'); this.game.playerScore += enemy.score; enemy.delete = true; }
  bossDead(enemy) { this.game.playerScore += enemy.score; this.game.playSound('explosion'); for (let i = 0; i < 3; i++) this.explosions.push(new Explosion(this.game, Math.random() * enemy.width + enemy.x, Math.random() * enemy.height + enemy.y)); this.levelComplete = true; enemy.delete = true; }
  playerDead() {
    this.player.delete = true; this.game.lives--; this.game.playSound('explosion');
...
export class SpaceImpactGame {
  ...
  setupInput() {
    ...
    if (fireBtn) fireBtn.addEventListener("pointerdown", () => { if (!this.gameOver && !this.gamePause && !this.keys.space.pressed) { this.currentLevel.playerProjectiles.push(new Projectile(this, true, this.currentLevel.player)); this.playSound('shoot'); this.keys.space.pressed = true; } });
